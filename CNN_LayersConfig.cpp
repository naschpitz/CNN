    ...
    case LayerType::CONV: {
      ...
      if (current.h + 2 * padY < conv.filterH) {
        throw std::runtime_error("CNN layer " + std::to_string(i) +
                                 " (conv): "
                                 "input height (" +
                                 std::to_string(current.h) + ") + 2*padY (" + std::to_string(2 * padY) +
      ...
      if (current.w + 2 * padX < conv.filterW) {
        throw std::runtime_error("CNN layer " + std::to_string(i) +
                                 " (conv): "
                                 "input width (" +
                                 std::to_string(current.w) + ") + 2*padX (" + std::to_string(2 * padX) +
      ...
      if (outH == 0 || outW == 0) {
        throw std::runtime_error("CNN layer " + std::to_string(i) +
                                 " (conv): "
                                 "output size is zero. Input: " +
                                 std::to_string(current.h) + "x" + std::to_string(current.w) +
    ...
    case LayerType::POOL: {
      ...
      if (current.h < pool.poolH) {
        throw std::runtime_error("CNN layer " + std::to_string(i) +
                                 " (pool): "
                                 "input height (" +
                                 std::to_string(current.h) + ") < poolH (" + std::to_string(pool.poolH) + ")");
      }
      ...
      if (current.w < pool.poolW) {
        throw std::runtime_error("CNN layer " + std::to_string(i) +
                                 " (pool): "
                                 "input width (" +
                                 std::to_string(current.w) + ") < poolW (" + std::to_string(pool.poolW) + ")");
      }
