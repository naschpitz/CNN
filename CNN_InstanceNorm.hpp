...
namespace CNN
{
  // Instance Normalization: normalizes per-image spatial statistics (H×W) per channel.
  // Formerly called BatchNorm in this codebase.
  template <typename T>
  class InstanceNorm
  {
    public:
      // Forward pass (inference): uses running mean/var
      // Forward pass (training): if batchMean/batchVar/xNormalized provided, computes per-image spatial stats
      static Tensor3D<T> propagate(const Tensor3D<T>& input, const Shape3D& inputShape, NormParameters<T>& params,
                                   const NormLayerConfig& config, std::vector<T>* batchMean = nullptr,
